...
public enum TTTMove {
	...
	public static boolean wins(EnumSet<TTTMove> moves) {
		return (moves.containsAll(lineA) || moves.containsAll(lineB) || moves.containsAll(lineC)
				|| moves.containsAll(col0) || moves.containsAll(col1) || moves.containsAll(col2)
				|| moves.containsAll(diagD) || moves.containsAll(diagU));
	}
